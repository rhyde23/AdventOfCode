...
def validatePassport(passport) :
    for code in codes :
        if not (code+':' in passport) :
            return False
        else :
            if code == 'byr' :
                try :
                    byr = re.findall('byr\:[0-9]{4}\s{1}', passport)[0][:-1]
                    year = int(byr.split(':')[1])
                ...
                try :
                    iyr = re.findall('iyr\:[0-9]{4}\s{1}', passport)[0][:-1]
                    year = int(iyr.split(':')[1])
                ...
                try :
                    eyr = re.findall('eyr\:[0-9]{4}\s{1}', passport)[0][:-1]
                    year = int(eyr.split(':')[1])
                ...
                try :
                    hgt = re.findall('hgt\:[0-9]+[a-z]{2}\s{1}', passport)[0][:-1]
                    measurement_system = hgt[-2:]
                    measurement = int(hgt.split(':')[1][:-2])
                    if measurement_system == 'cm' :
                        if measurement < 150 or measurement > 193 :
                            return False
                    elif measurement_system == 'in' :
                ...
                try :
                    hcl = re.findall('hcl\:\#[0-9a-f]{6}\s{1}', passport)[0][:-1]
                except :
                    return False
            elif code == 'ecl' :
                try :
                    eye_colors = ['amb','blu','brn','gry','grn','hzl','oth']
                    ecl = re.findall('ecl\:[a-z]{3}\s{1}', passport)[0][:-1]
                    if not (ecl.split(':')[1] in eye_colors) :
                        return False
                except :
                    return False
            elif code == 'pid' :
                try :
                    hcl = re.findall('pid\:[0-9]{9}\s{1}', passport)[0][:-1]
                except :
                    return False
    return True
